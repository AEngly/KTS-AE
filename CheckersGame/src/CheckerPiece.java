...
public class CheckerPiece extends Piece {
	...
	protected boolean isValidMove(Coord dest, Board board, Player playerTurn) {
		boolean m = (	this.colour == playerTurn //must be players own piece
						...
						&& ((Math.abs(this.getPos().getX()-dest.getX())==1 && Math.abs(this.getPos().getY()-dest.getY())==(playerTurn.getColour()=="White"? -1:1))
						||	(Math.abs(this.getPos().getX()-dest.getX())==2 && Math.abs(this.getPos().getY()-dest.getY())==(playerTurn.getColour()=="White"? -2:2)
